...
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  } else if (effectiveVariant === 'indigo' || effectiveVariant === 'info') {
      ...
      >
        ...
        <div className="flex items-start gap-4">
          <div className={`p-3 rounded-2xl border shrink-0 ${iconCls}`}>
            {effectiveVariant === 'danger' ? <Trash2 className="w-6 h-6" /> : <AlertTriangle className="w-6 h-6" />}
          </div>

          <div className="flex-1 pr-6">
            <h3 className="text-lg font-bold tracking-tight text-surface-900 dark:text-white">{title}</h3>
            {subtitle && <p className="mt-0.5 text-xs font-semibold text-surface-500 dark:text-surface-400">{subtitle}</p>}
